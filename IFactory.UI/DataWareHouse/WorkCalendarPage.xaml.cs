...
namespace IFactory.UI.DataWareHouse
{
    ...
    /// Page1.xaml 的交互逻辑
    /// </summary>
    public partial class WorkCalendar : BasePage, IComponentConnector
    {
        ...
        {
            DataPicResponse datapicResponse = LocalApi.WorkCalendar(new DataPicRequest() { });
            arrayDate = datapicResponse.DataPics.Select(m => m.ProductTime).ToArray();
        }

        private void LoadWorkDays()
        {
            DataPicResponse datapicResponse2 = LocalApi.WorkDays(new DataPicRequest() { });
            arrayDays = datapicResponse2.DataPics.Select(m => m.ProductTime).ToArray();
        }

        private void LoadWorkhours()
        {
            DataPicResponse datapicResponse2 = LocalApi.Workhours(new DataPicRequest() { });

...
{
    /// <summary>
    /// CraftMyLarPage.xaml 的交互逻辑
    /// </summary>
    public partial class CraftMyLarPage : BaseCraftIndexPage, IComponentConnector
    {
        public CraftMyLarPage()
        {
            InitializeComponent();
        ...
        {
            foreach (ProcessModel process in (await ClientHelper.ExecuteAsync<ProcessListResponse>(new ProcessListRequest() { CraftDID = this.CraftDID })).Processes)
            {

...
{
    /// <summary>
    /// CraftXRayPage.xaml 的交互逻辑
    /// </summary>
    public partial class CraftXRayPage : BaseCraftIndexPage, IComponentConnector
    {
        public CraftXRayPage()
        {
            InitializeComponent();
        ...
        public async void RefreshStates()
        {
            foreach (ProcessModel process in (IEnumerable<ProcessModel>)(await ClientHelper.ExecuteAsync<ProcessListResponse>((IRequest<ProcessListResponse>)new ProcessListRequest() { CraftDID = this.CraftDID })).Processes)
            {
                ...
                StateCycle.StateCycleItem stateCycleItem = this.stateCycleItems.FirstOrDefault((Func<StateCycle.StateCycleItem, bool>)(m => m.Name == shortNo));

...
{
    /// <summary>
    /// CraftBakingPage.xaml 的交互逻辑
    /// </summary>
    public partial class CraftBakingPage : BaseCraftIndexPage, IComponentConnector
    {
        public CraftBakingPage()
        {
            InitializeComponent();
        ...
        {
            foreach (ProcessModel process in (await ClientHelper.ExecuteAsync<ProcessListResponse>((IRequest<ProcessListResponse>)new ProcessListRequest() { CraftDID = this.CraftDID })).Processes)
            {

...
namespace IFactory.LocalService
{
    public class ProductionService : BaseService, IProductionService
    {
        ...
        {
            //return this.DataContext.CraftInfos.Join(this.DataContext.CraftProbablyInfos, c => c.DID, cp => cp.DID, (c, cp) => new CraftDetailModel()
            //{
            //    DID = c.DID,
            //    Name = c.Name,
            //    NO = c.NO,
        ...
        {
            //return this.DataContext.CraftInfos.Join(this.DataContext.CraftProbablyInfos, c => c.DID, cp => cp.DID, (c, cp) => new { c = c, cp = cp }).Where(data => data.c.DID == craftDID).Select(data => new CraftDetailModel()
            //{
            //    DID = data.c.DID,
            //    Name = data.c.Name,
            //    NO = data.c.NO,
